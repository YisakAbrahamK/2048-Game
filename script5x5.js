...
function shiftLeft(){ //lets assume the first row contain [0,4,4,8]
    for(let x=0;x<rowNum;x++){
...
function shiftRight(){ 
    for(let x=0;x<rowNum;x++){
        ...
        row=row.reverse(); // By just reversing we can use same function as shiftleft
        ...
        cells[x]=row.reverse(); // When we reverse it back we get shiftright
    }
}
...
function shiftUp(){ 
    for(let x=0;x<columnNum;x++){
        let column=[cells[0][x],cells[1][x],cells[2][x],cells[3][x],cells[4][x]];
...
function shiftDown(){ 
    for(let x=0;x<columnNum;x++){
        let column=[cells[0][x],cells[1][x],cells[2][x],cells[3][x],cells[4][x]].reverse();
        column=column.filter((num)=>num!=0); 
        for(let y=0;y<column.length-1;y++){ 
            if(column[y]===column[y+1]){
                column[y]=column[y]*2;
